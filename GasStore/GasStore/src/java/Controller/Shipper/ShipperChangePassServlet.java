 ...
 * @author Admin
 */
@WebServlet(name = "ShipperChangePassServlet", urlPatterns = {"/ShipperChangePass"})
public class ShipperChangePassServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ShipperChangePassServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ShipperChangePassServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (account != null) {
            ...
            if (sDAO.changePassword(MaHoa.toSHA1(newpass), uid)) {
                request.setAttribute("mess", "Change password success");
            } else {
                request.setAttribute("mess", "Change password Fail");
            }

            session.setAttribute("account", sDAO.getAccountById(account.getShipperID()));
            request.getRequestDispatcher("ProfileAdmin.jsp").forward(request, response);
        } else {
